...
### 5.2 地標(手工做到一眼認得出 — 盡量收齊台北)

方針:**台北招牌地標盡量收齊**,沿途當「邊滾邊吸收的 mini-goal」豐富旅程;台北 101 仍是**唯一終點 monument 與勝利過場**。地標皆為手搭幾何(非 OSM),沿用原版 curated singleton 機制(`addExtra` + `cityMap.LANDMARKS` + pack-scoped code),依大小落在對應尺度帶,`dioramaR` 吸收門檻(`dioramaR / ABSORB_RATIO`)嚴格遞增(101 最高、排最後)。位置為配速手擺,非真實相對地理。

**核心集(ship-blocking,必須做到精緻):**

| 地標 | 概略尺度帶 | 辨識特徵 |
|---|---|---|
| 北門(承恩門) | T3–T4 | 修復後的清代石砌城門 |
| 龍山寺 | T4 | 萬華廟宇,層疊燕尾脊 |
| 西門紅樓 | T4 | 八角紅磚 |
| 圓山大飯店 | T5 | 紅柱黃瓦宮殿式 |
| 總統府 | T5 | 紅白巴洛克 + 中央塔樓 |
| 中正紀念堂 + 自由廣場牌樓 | T5 | 藍頂白牆 + 五拱牌樓 |
| 台北小巨蛋 | T5 | 銀色巨蛋 |
| 台北 101(終點) | T6 | 八節竹節 + 金色阻尼器球 |

**延伸集(同機制,Phase 1 內逐步加,愈多愈好):**
國父紀念館、故宮博物院、台北車站(紅頂量體)、行天宮、兩廳院(國家音樂廳/戲劇院)、美麗華摩天輪、彩虹橋(基隆河)、貓空纜車站、新光三越站前(舊第一高)、南山廣場、大稻埕碼頭/迪化街、寶藏巖、士林官邸、林安泰古厝。

**引擎影響(實作注意):**
- 地標數從原版 11 大幅增加 → 需調高 `EXTRA_POOL_CAPS` 各 size-class 的**同時存活**上限(landmark-mid / large / xl);真正限制是「載入半徑內同時 alive 數」,地標散佈全圖故不會全部同時 alive,draw call 由 batched pool 共用、不隨地標數線性增加。
- `dioramaR` 門檻階梯要在這個更大集合上維持嚴格遞增;同帶多地標時靠座標分散避免擠在同一吸收點。
- **每個地標 = 一個手搭、要一眼認得出的模型 → 這是本專案最大的美術工時來源。** 故分核心集(擋上線)+ 延伸集(上線後/收尾迭代加),避免 Phase 1 卡在 20+ 個模型才能出。
...
| 本期關 | 台北 | 否(已定) |
| 黑熊名 | 月牙 | 是 |
| 地標 | 盡量收齊台北(核心 8 + 延伸 14,見 §5.2) | 是(可增刪;核心集擋上線、延伸集迭代加) |
| 收藏冊 | 上列 13 格 | 是 |
