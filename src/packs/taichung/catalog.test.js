...
describe('taipei catalog surface', () => {
  it('merges chunk + Tokyo EXTRA into a full CATALOG keyed by id (99 ids resolve)', () => {
    ...
    expect(CATALOG['caowu_path']).toBeDefined();
  });
  ...
  it('DISPLAY_NAME_BY_CODE has zh-TW names at every code (de-Tokyo: no Tokyo names left)', () => {
    ...
    // code 0 = T0 slot 0 = bubble_tea_seal = 珍奶封膜 (台中在地化)
    expect(DISPLAY_NAME_BY_CODE[0]).toBe('珍奶封膜');
    ...
    // code 93 is a Taichung extended landmark.
    expect(DISPLAY_NAME_BY_CODE[93]).toBe('草悟道');
  });
});
