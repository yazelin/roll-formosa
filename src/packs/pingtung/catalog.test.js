...
describe('taipei catalog surface', () => {
  it('merges chunk + Tokyo EXTRA into a full CATALOG keyed by id (99 ids resolve)', () => {
    ...
    // P7: Pingtung collectibles resolve (codes 70 + 94).
    expect(CATALOG['black_bear']).toBeDefined();
    expect(CATALOG['wanjin_cross']).toBeDefined();
  });
  ...
  it('DISPLAY_NAME_BY_CODE has zh-TW names at every code (de-Tokyo: no Tokyo names left)', () => {
    ...
    // code 0 = T0 slot 0 = coral_fragment = 珊瑚碎片 (Pingtung localized)
    expect(DISPLAY_NAME_BY_CODE[0]).toBe('珊瑚碎片');
    ...
    // code 93 is now a Pingtung extended landmark.
    expect(DISPLAY_NAME_BY_CODE[93]).toBe('墾丁凱撒飯店');
  });
});
