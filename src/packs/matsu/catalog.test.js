...
describe('matsu chunk archetypes', () => {
...
describe('matsu catalog surface', () => {
  it('merges chunk + EXTRA into a full CATALOG keyed by id (99 ids resolve)', () => {
    ...
    // Matsu collectibles resolve (codes 70 + 94).
    expect(CATALOG['black_bear']).toBeDefined();
    expect(CATALOG['mazu_statue']).toBeDefined();
    expect(CATALOG['blue_tears']).toBeDefined();
  });
  ...
  it('DISPLAY_NAME_BY_CODE has zh-TW names at every code (no Japanese kana)', () => {
    ...
    // EVERY code (0..98) is non-empty AND contains no Japanese kana.
    const kana = /[぀-ゟ゠-ヿ]/;
    for (let c = 0; c < 99; c++) {
    ...
    // code 93 is a Matsu extended landmark placeholder.
    expect(DISPLAY_NAME_BY_CODE[93]).toBe('擴充地標93');
  });
});
