/**
 * @file packs/matsu/archetypes/t0.js — Roll Formosa Matsu pack, TIER 0
 *   柑仔店與漁村桌頭 (corner-store & fishing village tabletop trinkets),
 *   the smallest absorbable band (radiusNominal 0.01–0.05 m).
 *   10 ArchetypeDefs, ids FROZEN by packs/matsu/tiers.js TIERS[0].archetypeIds
...
export const T0_ARCHETYPES = [
  ...
  /* ---------------------------------------------------------------- */
  /* [3] seashell 貝殼 — small spiral seashell (conch shape)            */
  /* ---------------------------------------------------------------- */
  {
    id: 'seashell',
    displayName: '貝殼',
    ...
    radiusJitter: 0.2,
    spawnWeight: 1.0,
    palette: [0xf4e8d8, 0xe8d4c4, 0xf0dcc8, 0xdcc8b8, 0xfff0e4],
    yOffset: -0.55,
    upright: false,
    collisionScale: 0.8,
    buildGeometry(rng) {
      const parts = [];
      // spiral shell body - layered cones creating spiral effect
      parts.push(cone(0.9, 1.2, 8, 0xffffff, { y: 0.6, hex2: 0xf0dcc8 })); // main shell cone
      parts.push(cone(0.7, 0.9, 8, 0xf4e8d8, { y: 0.9, rz: 0.1 })); // inner spiral
      parts.push(cone(0.5, 0.6, 7, 0xe8d4c4, { y: 1.1, rz: 0.2 })); // apex spiral
      parts.push(cone(0.3, 0.3, 6, 0xfff0e4, { y: 1.3 })); // tip
      // shell opening
      parts.push(sph(0.4, 0xffeedd, { ws: 6, hs: 4, y: 0.2, z: 0.4, sx: 0.8 }));
      // ridges on the shell
      for (let i = 0; i < 4; i++) {
        const a = (i / 4) * PI * 2;
        parts.push(box(0.08, 0.8, 0.1, 0xe0d0c0, { x: Math.cos(a) * 0.5, z: Math.sin(a) * 0.5, y: 0.7, ry: a }));
      }
      return finish(parts);
    },
  },
  ...
  /* ---------------------------------------------------------------- */
  /* [4] jiguang_biscuit 繼光餅 — traditional round biscuit with hole   */
  /* ---------------------------------------------------------------- */
  {
    id: 'jiguang_biscuit',
    displayName: '繼光餅',
    ...
    radiusJitter: 0.15,
    spawnWeight: 1.0,
    palette: [0xd4a050, 0xc89040, 0xe0b060, 0xb88030, 0xf0c878],
    yOffset: -0.75,
    upright: false,
    collisionScale: 0.85,
    buildGeometry(rng) {
      const parts = [];
      // main biscuit body - flattened torus shape (round with hole)
      parts.push(torus(0.7, 0.35, 5, 6, 0xffffff, { y: 0.35, rx: HALF_PI })); // main ring
      // browned top surface
      parts.push(cyl(0.95, 0.95, 0.08, 6, 0xd4a050, { y: 0.65 })); // top crust
      // sesame seeds scattered on top (reduced to 3)
      const seeds = 3;
      for (let i = 0; i < seeds; i++) {
        const a = (i / seeds) * PI * 2 + rng() * 0.3;
        const r = 0.5 + rng() * 0.25;
        parts.push(sph(0.06, 0xf8f0d8, { ws: 3, hs: 2, x: Math.cos(a) * r, z: Math.sin(a) * r, y: 0.7 }));
      }
      // bottom crust
      parts.push(cyl(0.95, 0.9, 0.06, 6, 0xb88030, { y: 0.05 }));
      return finish(parts);
    },
  },
  ...
  /* ---------------------------------------------------------------- */
  {
    ...
    collisionScale: 0.82,
    buildGeometry(rng) {
      const bronze = 0xffffff; // tinted bronze
      const parts = [
        // base foot
