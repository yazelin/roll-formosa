...
describe('nantou chunk archetypes', () => {
...
describe('nantou catalog surface', () => {
  it('merges chunk + Nantou EXTRA into a full CATALOG keyed by id (99 ids resolve)', () => {
    ...
    // Nantou collectibles resolve (codes 70 + 94).
    expect(CATALOG['black_bear']).toBeDefined();
    expect(CATALOG['mazu']).toBeDefined();
    // Nantou landmarks (code 82 = wenwu_temple)
    expect(CATALOG['wenwu_temple']).toBeDefined();
  });
  ...
  it('DISPLAY_NAME_BY_CODE has zh-TW names at every code (de-Tokyo: no Tokyo names left)', () => {
    ...
    // code 0 = T0 slot 0 = plum_candy = 梅子糖 (南投在地化)
    expect(DISPLAY_NAME_BY_CODE[0]).toBe('梅子糖');
    ...
    // code 93 is now a Nantou extended landmark (cien_pagoda variant).
    expect(DISPLAY_NAME_BY_CODE[93]).toBe('慈恩塔');
  });
});
