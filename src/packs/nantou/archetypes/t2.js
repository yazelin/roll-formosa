/**
 * @file packs/nantou/archetypes/t2.js — Roll Formosa Nantou pack, TIER 2 「竹山老街騎樓」.
 *
 * The 10 Zhushan old-street arcade archetypes, authored in the FROZEN id
 * order of tiers.js T2.archetypeIds (slots [0..7] absorbable, slots [8..9] are
 * the repeatable CHUNK LANDMARKS — bamboo_craft_cart / temple_incense_burner — at
...
export const T2_ARCHETYPES = [
  ...
  /* ---- slot 6 ---- 竹編籃 — bamboo woven basket (Zhushan specialty) */
  {
    id: 'bamboo_basket',
    displayName: '竹編籃',
    tier: 2,
    ...
    radiusJitter: 0.14,
    spawnWeight: 1.0,
    palette: [0xb09060, 0x8a7050, 0xc8a878, 0x9a7a58, 0x6a5040],
    yOffset: -0.2,
    upright: true,
    collisionScale: 0.82,
    buildGeometry(rng) {
      return finish([
        ...
        cyl(0.35, 0.5, 0.7, 8, 0xb09060, { y: 0.35, hex2: 0x8a7050 }),
        // rim binding (torus around top edge)
        torus(0.5, 0.04, 3, 8, 0x9a7a58, { rx: HALF_PI, y: 0.72 }),
        // bottom base (flat woven bottom)
        cyl(0.35, 0.35, 0.06, 8, 0xc8a878, { y: 0.03 }),
        // handle (arched over the basket)
        torus(0.35, 0.035, 3, 6, 0xb09060, {
          rx: HALF_PI,
          y: 0.95,
          theta0: 0,
          thetaLen: PI,
        ...
        cyl(0.45, 0.45, 0.03, 8, 0xc8a878, { y: 0.4 }),
      ]);
    },
  },

  /* ---- slot 7 ---- 竹筒 — decorative bamboo tube segment */
  {
    id: 'bamboo_tube',
    displayName: '竹筒',
    tier: 2,
    ...
    radiusJitter: 0.16,
    spawnWeight: 1.0,
    palette: [0x7a9050, 0x5a7030, 0x9ab060, 0x4a6020, 0xc8c890],
    yOffset: -0.06,
    upright: true,
    collisionScale: 0.75,
    buildGeometry(rng) {
      return finish([
        // main tube body (green bamboo)
        cyl(0.4, 0.4, 1.8, 8, 0x7a9050, { y: 0.9, hex2: 0x9ab060 }),
        // node rings (bamboo joints)
        cyl(0.44, 0.44, 0.08, 8, 0x5a7030, { y: 0.3 }),
        cyl(0.44, 0.44, 0.08, 8, 0x5a7030, { y: 0.9 }),
        cyl(0.44, 0.44, 0.08, 8, 0x5a7030, { y: 1.5 }),
        // top opening (light inner wood)
        cyl(0.35, 0.35, 0.08, 8, 0xc8c890, { y: 1.78 }),
        // bottom sealed (darker)
        cyl(0.4, 0.4, 0.06, 8, 0x4a6020, { y: 0.03 }),
      ]);
    },
  },

  /* ---- slot 8 (CHUNK LANDMARK) ---- 竹藝推車 — bamboo craft vendor cart */
  {
    id: 'bamboo_craft_cart',
    displayName: '竹藝推車',
    tier: 2,
    naturalBand: 2,
    radiusNominal: 1.8,
    radiusJitter: 0.12,
    spawnWeight: 0.3,
    palette: [0xb09060, 0x8a6a4a, 0x6a4a30, 0xd8d0c0, 0x5a7030],
    yOffset: -0.369,
    upright: true,
    collisionScale: 0.85,
    buildGeometry(rng) {
      ...
      return finish([
        ...
        // counter / display top
        box(2.15, 0.1, 1.25, 0xd8d0c0, { y: 1.3 }),
        // bamboo product display (stacked baskets)
        cyl(0.3, 0.35, 0.4, 8, 0xb09060, { x: -0.6, y: 1.55 }),
        cyl(0.25, 0.3, 0.35, 8, 0xb09060, { x: 0, y: 1.52 }),
        cyl(0.28, 0.32, 0.38, 8, 0xb09060, { x: 0.6, y: 1.54 }),
        // front sign panel (bamboo crafts)
        box(2.0, 0.5, 0.06, 0x5a7030, { y: 0.7, z: 0.56 }),
        // four roof posts
        cyl(0.06, 0.06, 1.2, 5, wood, { x: -0.85, y: 1.95, z: 0.45 }),
        cyl(0.06, 0.06, 1.2, 5, wood, { x: 0.85, y: 1.95, z: 0.45 }),
        cyl(0.06, 0.06, 1.2, 5, wood, { x: -0.85, y: 1.95, z: -0.45 }),
        cyl(0.06, 0.06, 1.2, 5, wood, { x: 0.85, y: 1.95, z: -0.45 }),
        // bamboo-style awning roof
        box(2.4, 0.12, 1.5, 0xb09060, { y: 2.6, rx: 0.0 }),
        // valance strip
        box(2.4, 0.28, 0.06, 0x6a4a30, { y: 2.42, z: 0.74 }),
        // two cart wheels
  ...
  /* ---- slot 9 (CHUNK LANDMARK) ---- 廟前香爐(鼎) — the bronze temple incense burner */
  {
