/**
 * @file catalog.js — Chiayi pack catalog.
 *
 * Assembles the 70 Chiayi chunk ArchetypeDefs from the 7 per-tier files,
 * then merges the EXTRA/v5 placeholders so all 99 codes still resolve.
 *
 * P6b: overrides EXTRA codes 82..89 with the 8 curated Chiayi landmark
 * geometries. Codes 70..81 (collectibles), 90..92 (shop/bridge/
 * tower slot), 93 (goal display slot), and v5 94..98 are unchanged placeholders.
 *
 * Exports (P5/P6b shape):
 *   CHUNK_ARCHETYPES  Record<id, ArchetypeDef> — 70 Chiayi chunk archetypes
 *   CATALOG           Record<id, ArchetypeDef> — 70 chunk + 29 EXTRA/v5 (99 ids)
 *   DISPLAY_NAME_BY_CODE  string[99] — indices 0..69 zh-TW, 70..98 names
 *   EXTRA_CATALOG, EXTRA_SIZE_CLASS_BY_CODE, EXTRA_POOL_CAPS — Chiayi-native
 *     for codes 82..89; remainder from the legacy engine catalog
 */

import { T0_ARCHETYPES } from './archetypes/t0.js';
import { T1_ARCHETYPES } from './archetypes/t1.js';
import { T2_ARCHETYPES } from './archetypes/t2.js';
import { T3_ARCHETYPES } from './archetypes/t3.js';
import { T4_ARCHETYPES } from './archetypes/t4.js';
import { T5_ARCHETYPES } from './archetypes/t5.js';
import { T6_ARCHETYPES } from './archetypes/t6.js';

import { HERO_TRI_CAP } from '../../config/tuning.js';

import { EXTRA_CODE_BASE } from '../../world/objects.js';

// 8 curated Chiayi landmark geometry descriptors (codes 82..89).
import { NM_SACRED_TREE } from './landmarks/sacred_tree.js';
import { NM_HINOKI_VILLAGE } from './landmarks/hinoki_village.js';
import { NM_CHIAYI_STATION } from './landmarks/chiayi_station.js';
import { NM_CENTRAL_FOUNTAIN } from './landmarks/central_fountain.js';
import { NM_WENHUA_NIGHT_MARKET } from './landmarks/wenhua_night_market.js';
import { NM_CHIAYI_PARK } from './landmarks/chiayi_park.js';
import { NM_ALISHAN_TRAIN } from './landmarks/alishan_train.js';
import { NM_BEIGANG_CHAOTIAN } from './landmarks/beigang_chaotian.js';

// 13 Chiayi collectible (rare album) geometries (codes 70..81 + 94).
import { COL_BLACK_BEAR } from './collectibles/black_bear.js';
import { COL_TURKEY_RICE } from './collectibles/turkey_rice.js';
import { COL_FANGKUAISU } from './collectibles/fangkuaisu.js';
import { COL_HINOKI_BENTO } from './collectibles/hinoki_bento.js';
import { COL_ALISHAN_TEA } from './collectibles/alishan_tea.js';
import { COL_MOCHI } from './collectibles/mochi.js';
import { COL_SANTAIZI } from './collectibles/santaizi.js';
import { COL_BUDAIXI } from './collectibles/budaixi.js';
import { COL_FOREST_TRAIN } from './collectibles/forest_train.js';
import { COL_EGG_PANCAKE } from './collectibles/egg_pancake.js';
import { COL_MILKFISH } from './collectibles/milkfish.js';
import { COL_STONE_MONKEY } from './collectibles/stone_monkey.js';
import { COL_MAZU } from './collectibles/mazu.js';

// Extended landmarks (codes 90-93 + 95-98) - reuse goal monument for now
import { NM_SUN_SHOOTING_TOWER } from './landmarks/sun_shooting_tower.js';

/* ================================================================== */
/* 70 chunk archetypes, assembled in tier order (code = tier*10 + slot)*/
/* ================================================================== */

/** @type {import('../../types.js').Archetype[]} */
const _allTierArchetypes = [
  ...T0_ARCHETYPES,
  ...T1_ARCHETYPES,
  ...T2_ARCHETYPES,
  ...T3_ARCHETYPES,
  ...T4_ARCHETYPES,
  ...T5_ARCHETYPES,
  ...T6_ARCHETYPES,
];

if (_allTierArchetypes.length !== 70) {
  throw new Error(
    `[chiayi/catalog] expected exactly 70 chunk archetypes, got ${_allTierArchetypes.length}`
  );
}

/**
 * Chiayi chunk archetypes keyed by id — 70 entries (one per slot across 7 tiers).
 * Code i = the archetype at array position i (tier*10 + slot within tier).
 * @type {Record<string, import('../../types.js').Archetype>}
 */
export const CHUNK_ARCHETYPES = {};
for (const arch of _allTierArchetypes) {
  CHUNK_ARCHETYPES[arch.id] = arch;
}

/* ================================================================== */
/* CATALOG — 70 chunk + 29 EXTRA/v5 placeholders = 99 ids             */
/* ================================================================== */

/**
 * The 8 Chiayi curated landmark geometry descriptors (codes 82..89).
 * These REPLACE the placeholder landmark ids at the same codes.
 */
const _CHIAYI_LANDMARKS = [
  { code: 82, nm: NM_SACRED_TREE,        sizeClass: 'landmark-mid',   tier: 3, naturalBand: 3 },
  { code: 83, nm: NM_HINOKI_VILLAGE,     sizeClass: 'landmark-mid',   tier: 3, naturalBand: 3 },
  { code: 84, nm: NM_CHIAYI_STATION,     sizeClass: 'landmark-mid',   tier: 3, naturalBand: 3 },
  { code: 85, nm: NM_CENTRAL_FOUNTAIN,   sizeClass: 'landmark-large', tier: 4, naturalBand: 4 },
  { code: 86, nm: NM_WENHUA_NIGHT_MARKET,sizeClass: 'landmark-mid',   tier: 4, naturalBand: 4 },
  { code: 87, nm: NM_CHIAYI_PARK,        sizeClass: 'landmark-large', tier: 5, naturalBand: 5 },
  { code: 88, nm: NM_ALISHAN_TRAIN,      sizeClass: 'landmark-large', tier: 5, naturalBand: 5 },
  { code: 89, nm: NM_BEIGANG_CHAOTIAN,   sizeClass: 'landmark-large', tier: 5, naturalBand: 5 },
];

/**
 * Full id-keyed catalog: 70 Chiayi chunk archetypes PLUS the 24 EXTRA
 * (codes 70..93) and 5 v5 (codes 94..98). Codes 82..89 are replaced with
 * Chiayi landmark geometries; all others carry placeholder archetypes.
 * Total: exactly 99 ids.
 * @type {Record<string, import('../../types.js').Archetype>}
 */
export const CATALOG = { ...CHUNK_ARCHETYPES };

/* ================================================================== */
/* EXTRA_CATALOG, EXTRA_SIZE_CLASS_BY_CODE, EXTRA_POOL_CAPS            */
/* ================================================================== */

/**
 * EXTRA archetypes keyed by frozen code (70..93 + v5 94..98). Every code is a
 * Chiayi collectible (70..81+94) or landmark (82..93+95..98), filled by the
 * registration loops below.
 * @type {Record<number, import('../../types.js').Archetype & {extraCode:number, sizeClass:string|null}>}
 */
export const EXTRA_CATALOG = {};

/**
 * Size-class pool assignment per EXTRA code (filled by the loops below).
 * @type {Record<number, string|null>}
 */
export const EXTRA_SIZE_CLASS_BY_CODE = {};

for (const { code, nm, sizeClass, tier, naturalBand } of _CHIAYI_LANDMARKS) {
  const _g = nm.buildGeometry(() => 0.5);
  _g.computeBoundingBox();
  const _yOffset = -1 - _g.boundingBox.min.y;
  if (_g.dispose) _g.dispose();
  /** @type {import('../../types.js').Archetype & {extraCode:number, sizeClass:string}} */
  const entry = {
    id: nm.id,
    displayName: nm.name,
    tier,
    naturalBand,
    radiusNominal: nm.dioramaRHint,
    radiusJitter: 0,
    spawnWeight: 0, // curated-only — never random-rolled
    palette: [nm.colorHex],
    yOffset: _yOffset,
    upright: true,
    collisionScale: 1.0,
    heroTriCap: HERO_TRI_CAP, // landmark geometries use the 600-tri hero budget
    buildGeometry: nm.buildGeometry.bind(nm),
    extraCode: code,
    sizeClass,
  };
  CATALOG[nm.id] = entry;
  EXTRA_CATALOG[code] = entry;
  EXTRA_SIZE_CLASS_BY_CODE[code] = sizeClass;
}

/* 13 Chiayi collectibles at codes 70..81 + 94. */
const _CHIAYI_COLLECTIBLES = [
  { code: 70, col: COL_BLACK_BEAR },
  { code: 71, col: COL_TURKEY_RICE },
  { code: 72, col: COL_FANGKUAISU },
  { code: 73, col: COL_HINOKI_BENTO },
  { code: 74, col: COL_ALISHAN_TEA },
  { code: 75, col: COL_MOCHI },
  { code: 76, col: COL_SANTAIZI },
  { code: 77, col: COL_BUDAIXI },
  { code: 78, col: COL_FOREST_TRAIN },
  { code: 79, col: COL_EGG_PANCAKE },
  { code: 80, col: COL_MILKFISH },
  { code: 81, col: COL_STONE_MONKEY },
  { code: 94, col: COL_MAZU },
];
for (const { code, col } of _CHIAYI_COLLECTIBLES) {
  const _g = col.buildGeometry(() => 0.5);
  _g.computeBoundingBox();
  const _yOffset = -1 - _g.boundingBox.min.y;
  if (_g.dispose) _g.dispose();
  const entry = {
    id: col.id,
    displayName: col.name,
    tier: 1,
    naturalBand: 1,
    radiusNominal: 0.3,
    radiusJitter: 0,
    spawnWeight: 0, // curated-only — placed, never random-rolled
    palette: [col.colorHex],
    yOffset: _yOffset,
    upright: true,
    collisionScale: 1.0,
    heroTriCap: HERO_TRI_CAP, // collectibles get the 600-tri hero budget
    buildGeometry: col.buildGeometry.bind(col),
    extraCode: code,
    sizeClass: 'collectible-small',
  };
  CATALOG[col.id] = entry;
  EXTRA_CATALOG[code] = entry;
  EXTRA_SIZE_CLASS_BY_CODE[code] = 'collectible-small';
}

/* 8 Chiayi extended landmarks at codes 90-93 + 95-98 (placeholder with goal monument). */
const _CHIAYI_EXTRA_LANDMARKS = [
  { code: 90, nm: NM_SUN_SHOOTING_TOWER, sizeClass: 'landmark-xl',  tier: 5, naturalBand: 5 },
  { code: 91, nm: NM_SUN_SHOOTING_TOWER, sizeClass: 'landmark-xl',  tier: 5, naturalBand: 5 },
  { code: 92, nm: NM_SUN_SHOOTING_TOWER, sizeClass: 'landmark-xl',  tier: 5, naturalBand: 5 },
  { code: 93, nm: NM_SUN_SHOOTING_TOWER, sizeClass: 'landmark-xl',  tier: 5, naturalBand: 5 },
  { code: 95, nm: NM_SUN_SHOOTING_TOWER, sizeClass: 'landmark-mid', tier: 4, naturalBand: 4 },
  { code: 96, nm: NM_SUN_SHOOTING_TOWER, sizeClass: 'landmark-mid', tier: 5, naturalBand: 5 },
  { code: 97, nm: NM_SUN_SHOOTING_TOWER, sizeClass: 'landmark-mid', tier: 5, naturalBand: 5 },
  { code: 98, nm: NM_SUN_SHOOTING_TOWER, sizeClass: 'landmark-mid', tier: 4, naturalBand: 4 },
];
for (const { code, nm, sizeClass, tier, naturalBand } of _CHIAYI_EXTRA_LANDMARKS) {
  const _g = nm.buildGeometry(() => 0.5);
  _g.computeBoundingBox();
  const _yOffset = -1 - _g.boundingBox.min.y;
  if (_g.dispose) _g.dispose();
  const entry = {
    id: `${nm.id}_${code}`, // make unique id for each code
    displayName: nm.name,
    tier,
    naturalBand,
    radiusNominal: nm.dioramaRHint || (sizeClass === 'landmark-xl' ? 80 : 30),
    radiusJitter: 0,
    spawnWeight: 0, // curated-only — not placed yet
    palette: [nm.colorHex],
    yOffset: _yOffset,
    upright: true,
    collisionScale: 1.0,
    heroTriCap: HERO_TRI_CAP,
    buildGeometry: nm.buildGeometry.bind(nm),
    extraCode: code,
    sizeClass,
  };
  CATALOG[entry.id] = entry;
  EXTRA_CATALOG[code] = entry;
  EXTRA_SIZE_CLASS_BY_CODE[code] = sizeClass;
}

/**
 * EXTRA render pool caps. Chiayi has 8 landmark singletons:
 *   landmark-mid (codes 82/83/84/86): 4 alive (matches the original engine floor)
 *   landmark-large (codes 85/87/88/89): 4 alive
 * Caps are capacity only — same 4 batches, zero extra draws.
 * @type {Readonly<Record<string, number>>}
 */
export const EXTRA_POOL_CAPS = Object.freeze({
  'collectible-small': 13,
  'landmark-mid': 12,
  'landmark-large': 4,
  'landmark-xl': 4,
});

/* ================================================================== */
/* DISPLAY_NAME_BY_CODE — string[99]                                   */
/* ================================================================== */

/**
 * Code-indexed display names (99 total):
 *   0..69  — zh-TW names from the Chiayi chunk archetypes (displayName field)
 *   70..81 — collectible names
 *   82..89 — Chiayi landmark zh-TW names
 *   90..98 — extended landmark names
 * @type {string[]}
 */
export const DISPLAY_NAME_BY_CODE = (() => {
  const names = new Array(99);

  // Codes 0..69: zh-TW chunk names in tier-major, slot-minor order.
  for (let i = 0; i < _allTierArchetypes.length; i++) {
    names[i] = _allTierArchetypes[i].displayName || '';
  }

  // Codes 70..98: init '' then override
  for (let c = EXTRA_CODE_BASE; c < 99; c++) {
    names[c] = '';
  }

  // Override codes 82..89 with Chiayi landmark zh-TW names.
  for (const { code, nm } of _CHIAYI_LANDMARKS) {
    names[code] = nm.name;
  }

  // Override codes 70..81 + 94 with Chiayi collectible zh-TW names.
  for (const { code, col } of _CHIAYI_COLLECTIBLES) {
    names[code] = col.name;
  }

  // codes 90..93 + 95..98 -> Chiayi extended landmark zh-TW names.
  for (const { code, nm } of _CHIAYI_EXTRA_LANDMARKS) {
    names[code] = nm.name;
  }

  return names;
})();

if (DISPLAY_NAME_BY_CODE.length !== 99) {
  throw new Error(
    `[chiayi/catalog] DISPLAY_NAME_BY_CODE must be 99 entries, got ${DISPLAY_NAME_BY_CODE.length}`
  );
}
