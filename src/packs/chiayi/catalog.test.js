...
describe('chiayi chunk archetypes', () => {
...
describe('chiayi catalog surface', () => {
  it('merges chunk + EXTRA into a full CATALOG keyed by id (99 ids resolve)', () => {
    // 70 chunk + 13 collectibles (70..81+94) + 8 landmarks (82..89) + 8 extended (90..93+95..98) = 99 ids.
    ...
    // Chiayi collectibles and landmarks resolve.
    expect(CATALOG['black_bear']).toBeDefined();
    expect(CATALOG['mazu']).toBeDefined();
    expect(CATALOG['sacred_tree']).toBeDefined();
  });
  ...
  it('DISPLAY_NAME_BY_CODE has zh-TW names at every code (de-Tokyo: no Japanese kana)', () => {
    ...
    // code 0 = T0 slot 0 = alishan_tea_bag = 阿里山茶包
    expect(DISPLAY_NAME_BY_CODE[0]).toBe('阿里山茶包');
    ...
    // code 93 is now a Chiayi extended landmark (shooting tower slot).
    expect(DISPLAY_NAME_BY_CODE[93]).toBe('射日塔');
  });
});
