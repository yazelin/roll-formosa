/**
 * @file packs/chiayi/tiers.js — Roll Formosa Chiayi pack: the 7-tier scale
 * ladder (圖釘 2 cm → 射日塔). Pack-scoped re-theme of the engine's legacy
 * tier table, themed as a WOOD-CITY / ALISHAN progression up the 檜意森活村 /
 * 中央噴水池 / 嘉義市區 skyline. The engine reads tiers via the active pack
 ...
 *
 * archetypeIds[10] per tier are the FROZEN CONTRACT with packs/chiayi/catalog.js
 * (Part 5): slots [0..7] absorbable, slots [8..9] repeatable CHUNK LANDMARKS.
 * The named singleton landmarks (檜意森活村 / 射日塔 …) are NOT here — they live in
 * packs/chiayi/landmarks.js + cityMap.js (curated EXTRA code space).
 *
 ...
 * (T0/T1, 文化路/嘉義夜市) → 漸冷霓虹紫 (T2-T4) → 冷電光夜店藍 (T5/T6, 射日塔電城).
...
export const TIERS = [
  {
    index: 0,
    name: '木都柑仔店桌頭', // 圖釘/文具桌頭 — drives HUD #tier-label
    enterTrueRadius: 0.02,
    cellSizeSim: 32,
    loadRadiusSim: 96,
    objectsPerChunk: 72,
    archetypeIds: [
      // 阿里山茶包, 檜木香包, 圖釘, 瓶蓋, 方塊酥, 火雞肉飯代幣, 檜木尺, 檜木杯墊
      'alishan_tea_bag', 'hinoki_sachet', 'pushpin', 'bottle_cap', 'fangkuaisu_stack', 'turkey_rice_token', 'hinoki_ruler', 'hinoki_coaster',
  ...
  },
  {
    ...
    objectsPerChunk: 72,
    archetypeIds: [
      // 雞肉飯, 涼麵, 火雞肉, 味噌湯, 沙鍋魚頭, 春捲, 紅白塑膠袋, 豆花
      'turkey_rice_bowl', 'liangmian', 'turkey_slice', 'miso_soup', 'fish_head_pot', 'spring_roll', 'redwhite_bag', 'douhua',
      // chunk landmarks: 夜市拱門, 攤車
      'wenhua_market_arch', 'food_stall',
    ],
    fogColor: 0x3e2026, // 文化路夜市黃昏暖霧
    skyTop: 0x1c0f14,
    skyBottom: 0x6e2c44,
  ...
  },
  {
    ...
    objectsPerChunk: 72,
    archetypeIds: [
      // 機車, 小貨車, 圓環花圃, 鐵捲門, 路燈柱, 諸羅樹蛙像, 榕樹, 石獅
      'scooter', 'mini_truck', 'roundabout_planter', 'roll_shutter', 'lamppost', 'tree_frog_statue', 'banyan_tree', 'stone_lion',
      // chunk landmarks: 噴水池圓環, 城隍廟牌樓
      'fountain_roundabout', 'chenghuang_pailou',
    ],
    fogColor: 0x2a1c3e, // 暮色中央噴水池藍起調
    skyTop: 0x130c24,
    skyBottom: 0x48287a,
  ...
  },
  {
    ...
    objectsPerChunk: 72,
    archetypeIds: [
      // 木造街屋, 鐵皮屋, 日治公寓, 超商, 嘉義BRT, 垃圾車, 加油站, 檜意騎樓
      'woodcity_townhouse', 'tin_roof_house', 'colonial_apartment', 'convenience_store', 'chiayi_brt', 'garbage_truck', 'gas_station', 'hinoki_arcade',
  ...
  },
  {
    ...
    objectsPerChunk: 72,
    archetypeIds: [
      // 林業商辦, 耐斯百貨, 嘉義車站, 車站跨站天橋, 木造停車場, 阿里山森鐵看板, 玻璃帷幕街屋, 嘉義銀行
      'forestry_office', 'nice_plaza', 'chiayi_station_plaza', 'station_overpass', 'wood_parking', 'alishan_billboard', 'glass_curtain_house', 'chiayi_bank',
  ...
  },
  {
    ...
    fogColor: 0x18183e, // 嘉義公園夜空 深藍紫 (the finale band)
    skyTop: 0x121238,
    skyBottom: 0x3030aa,
    sunDir: [-0.70, 0.08, 0.40],
    sunIntensity: 0.55, // 入夜 — dimmed; 射日塔點燈接手
...
export function validateTiersStructure() {
  const assert = (cond, msg) => {
    if (!cond) throw new Error(`[chiayi tiers] ${msg}`);
  };
  assert(TIERS.length === 7, 'exactly 7 tiers (圖釘→射日塔, x5 ladder)');
  const seen = new Set();
  for (let t = 0; t < TIERS.length; t++) {
    ...
    /* Sky/fog authoring guards (relocated from the engine's old config/tiers.js
       — they now validate the SHIPPING Chiayi tier params, not dead legacy data).
