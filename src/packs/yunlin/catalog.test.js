...
describe('yunlin chunk archetypes', () => {
...
describe('yunlin catalog surface', () => {
  it('merges chunk + Yunlin EXTRA into a full CATALOG keyed by id (99 ids resolve)', () => {
    ...
    // P7: Yunlin collectibles resolve (codes 70 + 94).
  ...
  it('DISPLAY_NAME_BY_CODE has zh-TW names at every code (de-Tokyo: no Tokyo names left)', () => {
    ...
    // code 0 = T0 slot 0 = peanut_shell_bit = 土豆殼碎
    expect(DISPLAY_NAME_BY_CODE[0]).toBe('土豆殼碎');
    ...
    // code 93 is now a Yunlin extended landmark (西螺大橋 variants).
    expect(DISPLAY_NAME_BY_CODE[93]).toBe('西螺大橋');
  });
});
