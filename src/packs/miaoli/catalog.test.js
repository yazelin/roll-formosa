...
describe('miaoli catalog surface', () => {
  it('merges chunk + Miaoli EXTRA into a full CATALOG keyed by id (99 ids resolve)', () => {
    ...
    // Miaoli collectibles resolve (codes 70..81 + 94).
    expect(CATALOG['black_bear']).toBeDefined();
    expect(CATALOG['leicha']).toBeDefined();
    expect(CATALOG['tung_oil']).toBeDefined();
  });
  ...
  it('DISPLAY_NAME_BY_CODE has zh-TW names at every code (de-Tokyo: no Tokyo names left)', () => {
    ...
    // code 0 = T0 slot 0 = tung_blossom_petal = 桐花瓣 (Miaoli's iconic 油桐花)
    expect(DISPLAY_NAME_BY_CODE[0]).toBe('桐花瓣');
    ...
    // code 82 = 勝興車站 (Shengxing Station landmark)
    expect(DISPLAY_NAME_BY_CODE[82]).toBe('勝興車站');
  });
});
