/**
 * @file packs/miaoli/archetypes/t2.js — Roll Formosa Miaoli pack, TIER 2 「草莓園騎樓」.
 *
 * The 10 arcade-sidewalk (騎樓) rollable archetypes for Miaoli's strawberry farm
 * and arcade scenery. Authored in the FROZEN id order of tiers.js T2.archetypeIds
 * (slots [0..7] absorbable, slots [8..9] are the repeatable CHUNK LANDMARKS —
 * strawberry_cart / temple_incense_burner — at spawnWeight ~0.3).
 *
 ...
 *
 * T2 theme: 草莓園騎樓 — Strawberry farm arcade items
 * Items: red_plastic_chair, helmet, strawberry_basket, gas_cylinder,
 *        traffic_cone, fire_hydrant, lucky_cat, strawberry_crate,
...
export const T2_ARCHETYPES = [
  ...
  /* ---- slot 2 ---- 草莓籃 — woven strawberry harvest basket (Dahu specialty)        */
  {
    id: 'strawberry_basket',
    displayName: '草莓籃',
    tier: 2,
    naturalBand: 2,
    radiusNominal: 0.35,
    radiusJitter: 0.15,
    spawnWeight: 1.0,
    palette: [0xc8a870, 0x9a7a4a, 0xb89050, 0x8a6a3a, 0xe8c890],
    yOffset: -0.30,
    upright: true,
    collisionScale: 0.85,
    buildGeometry(rng) {
      const parts = [
        // woven basket body (tapered cylinder with wicker texture via gradient)
        cyl(0.9, 0.6, 0.7, 10, 0xffffff, { y: 0.35, hex2: 0xc8a870 }),
        // basket rim
        cyl(0.95, 0.92, 0.12, 10, 0x9a7a4a, { y: 0.72, open: true }),
        // base ring
        cyl(0.62, 0.62, 0.08, 10, 0x8a6a3a, { y: 0.04 }),
        // handle arc
        torus(0.6, 0.06, 5, 8, 0x9a7a4a, { y: 1.1, arc: PI }),
      ];
      // pile of strawberries inside
      const berries = [
        [0, 0.8, 0, 0.2],
        [-0.3, 0.75, 0.15, 0.18],
        [0.28, 0.72, -0.1, 0.17],
      ...
      for (const [x, y, z, r] of berries) {
        parts.push(cone(r, r * 1.5, 6, 0xe83040, { x, y, z }));
        parts.push(cone(r * 0.4, r * 0.3, 4, 0x4a8a3a, { x, y: y + r * 0.7, z })); // calyx
      }
      return finish(parts);
    },
  },
  ...
  /* ---- slot 7 ---- 草莓箱 — stacked strawberry shipping crates (Dahu 大湖)          */
  {
    id: 'strawberry_crate',
    displayName: '草莓箱',
    tier: 2,
    ...
    radiusJitter: 0.14,
    spawnWeight: 1.0,
    palette: [0xe83040, 0xd02838, 0xf4f0e8, 0xff5060, 0x4a8a3a],
    yOffset: -0.35,
    upright: true,
    collisionScale: 0.82,
    buildGeometry(rng) {
      const parts = [
        // bottom crate (pink/red cardboard box)
        box(1.4, 0.5, 1.0, 0xffffff, { y: 0.25 }), // tinted pink/red
        // printed strawberry logo area
        box(1.0, 0.3, 0.02, 0x4a8a3a, { y: 0.25, z: 0.51 }),
        box(0.5, 0.2, 0.03, 0xf4f0e8, { y: 0.25, z: 0.52 }),
        // top crate (stacked)
        box(1.4, 0.5, 1.0, 0xe83040, { y: 0.75 }),
        // visible strawberries peeking from top crate
        cone(0.12, 0.18, 5, 0xe83040, { x: -0.4, y: 1.05, z: 0.2 }),
        cone(0.1, 0.15, 5, 0xf04050, { x: 0.3, y: 1.02, z: -0.15 }),
        cone(0.11, 0.16, 5, 0xd02838, { x: 0.0, y: 1.04, z: 0.0 }),
        // green calyx leaves on strawberries
        cone(0.05, 0.05, 4, 0x4a8a3a, { x: -0.4, y: 1.15, z: 0.2, rx: PI }),
        cone(0.04, 0.04, 4, 0x4a8a3a, { x: 0.3, y: 1.12, z: -0.15, rx: PI }),
        cone(0.045, 0.05, 4, 0x4a8a3a, { x: 0.0, y: 1.13, z: 0.0, rx: PI }),
      ];
      return finish(parts);
    },
  },

  /* ---- slot 8 (CHUNK LANDMARK) ---- 草莓攤車 — strawberry vendor cart with awning   */
  {
    id: 'strawberry_cart',
    displayName: '草莓攤車',
    tier: 2,
    ...
    radiusJitter: 0.12,
    spawnWeight: 0.3,
    palette: [0xe83050, 0xff6080, 0xf4f0e8, 0x4a8a3a, 0xc8a870],
    yOffset: -0.37,
    upright: true,
    collisionScale: 0.85,
    buildGeometry(rng) {
      ...
      const parts = [
        ...
        // counter / display top
        box(2.15, 0.1, 1.25, 0xf4f0e8, { y: 1.3 }),
        // front signboard (pink/red strawberry theme)
        box(2.0, 0.5, 0.06, 0xe83050, { y: 0.7, z: 0.56, hex2: 0xff6080 }),
        // strawberry logo on sign
        cone(0.2, 0.3, 6, 0xe83040, { y: 0.8, z: 0.6 }),
        // four roof posts
        cyl(0.06, 0.06, 1.2, 5, wood, { x: -0.85, y: 1.95, z: 0.45 }),
        cyl(0.06, 0.06, 1.2, 5, wood, { x: 0.85, y: 1.95, z: 0.45 }),
        cyl(0.06, 0.06, 1.2, 5, wood, { x: -0.85, y: 1.95, z: -0.45 }),
        cyl(0.06, 0.06, 1.2, 5, wood, { x: 0.85, y: 1.95, z: -0.45 }),
        // striped pink/white awning roof
        box(2.4, 0.12, 1.5, 0xe83050, { y: 2.6 }),
        box(0.4, 0.14, 1.5, 0xf4f0e8, { x: -0.6, y: 2.61 }), // white stripe
        box(0.4, 0.14, 1.5, 0xf4f0e8, { x: 0.6, y: 2.61 }),
        // strawberry baskets on counter
        cyl(0.35, 0.25, 0.25, 8, 0xc8a870, { x: -0.6, y: 1.48 }),
        cyl(0.35, 0.25, 0.25, 8, 0xc8a870, { x: 0.5, y: 1.48 }),
        // strawberries in baskets
        cone(0.1, 0.15, 5, 0xe83040, { x: -0.6, y: 1.7, z: 0.05 }),
        cone(0.1, 0.15, 5, 0xf04050, { x: 0.5, y: 1.68, z: -0.05 }),
        // two cart wheels
