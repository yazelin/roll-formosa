/**
 * @file packs/kinmen/archetypes/t2.js — Roll Formosa Kinmen pack, TIER 2 「閩南聚落」.
 *
 * The 10 Minnan village (閩南聚落) rollable archetypes, authored in the FROZEN id
 * order of tiers.js T2.archetypeIds (slots [0..7] absorbable, slots [8..9] are
 * the repeatable CHUNK LANDMARKS — minnan_stone_house / ancestral_pailou — at
...
export const T2_ARCHETYPES = [
  ...
  /* ---- slot 1 ---- 斗笠 — traditional bamboo conical hat                */
  {
    id: 'bamboo_hat',
    displayName: '斗笠',
    tier: 2,
  ...
  /* ---- slot 2 ---- 石臼 — stone mortar for grinding                     */
  {
    ...
    buildGeometry(rng) {
      ...
      parts.push(sph(0.15, 0x707068, { ws: 6, hs: 4, x: 1.15, y: 0.35 }));
      return finish(parts);
    },
  },
  ...
  /* ---- slot 6 ---- 風獅爺小像 — small wind lion god statue (Kinmen icon) */
  {
    id: 'small_wind_lion',
    displayName: '風獅爺小像',
    tier: 2,
    ...
    radiusJitter: 0.15,
    spawnWeight: 1.0,
    palette: [0xc8a050, 0xb89040, 0xd8b060, 0xa88030, 0xe8c070],
    yOffset: -0.18,
    upright: true,
    collisionScale: 0.82,
    buildGeometry(rng) {
      ...
      const parts = [];
      // base pedestal
      parts.push(box(0.7, 0.25, 0.5, 0x909088, { y: 0.12, hex2: 0xa0a098 }));
      // body - seated lion form
      parts.push(cyl(0.35, 0.4, 0.6, 8, stone, { y: 0.5, hex2: 0xd8b060 }));
      parts.push(sph(0.4, stone, { ws: 7, hs: 4, y: 0.85, sy: 0.7 }));
      // head - characteristic fierce face
      parts.push(sph(0.32, stone, { ws: 7, hs: 5, y: 1.2, hex2: 0xe8c070 }));
      // large open mouth (wind lion characteristic)
      parts.push(box(0.2, 0.12, 0.15, 0x802020, { y: 1.1, z: 0.28 }));
      // eyes
      parts.push(sph(0.06, 0x202020, { ws: 4, hs: 3, x: -0.1, y: 1.25, z: 0.25 }));
      parts.push(sph(0.06, 0x202020, { ws: 4, hs: 3, x: 0.1, y: 1.25, z: 0.25 }));
      // mane curls around head
      parts.push(sph(0.15, 0xb89040, { ws: 5, hs: 3, x: -0.25, y: 1.25 }));
      parts.push(sph(0.15, 0xb89040, { ws: 5, hs: 3, x: 0.25, y: 1.25 }));
      parts.push(sph(0.12, 0xa88030, { ws: 5, hs: 3, y: 1.42 }));
      // front paws
      parts.push(sph(0.12, stone, { ws: 5, hs: 3, x: -0.18, y: 0.38, z: 0.25 }));
      parts.push(sph(0.12, stone, { ws: 5, hs: 3, x: 0.18, y: 0.38, z: 0.25 }));
      // tail curl on back
      parts.push(torus(0.1, 0.04, 4, 6, stone, { x: 0, y: 1.0, z: -0.35, arc: PI }));
      return finish(parts);
    },
  },
  ...
  /* ---- slot 8 (CHUNK LANDMARK) ---- 閩南石屋 — Minnan traditional stone house */
  {
    id: 'minnan_stone_house',
    displayName: '閩南石屋',
    tier: 2,
    naturalBand: 2,
    radiusNominal: 1.8,
    ...
    buildGeometry(rng) {
      const stone = 0xffffff;
      const parts = [];
      // main house body - rectangular stone block (Kinmen granite style)
      ...
      // door frame - red lacquered wood (Kinmen style)
      parts.push(box(0.08, 1.0, 0.12, 0x8a2020, { x: -0.29, y: 0.5, z: 0.82 }));
      parts.push(box(0.08, 1.0, 0.12, 0x8a2020, { x: 0.29, y: 0.5, z: 0.82 }));
      parts.push(box(0.66, 0.08, 0.12, 0x8a2020, { y: 1.0, z: 0.82 }));
      // small window
      parts.push(box(0.35, 0.3, 0.1, 0x303840, { x: 0.7, y: 0.9, z: 0.81 }));
      // window frame
      parts.push(box(0.45, 0.05, 0.12, 0x8a2020, { x: 0.7, y: 1.08, z: 0.82 }));
      parts.push(box(0.45, 0.05, 0.12, 0x8a2020, { x: 0.7, y: 0.72, z: 0.82 }));
      // swallowtail roof (燕尾) - Kinmen/Fujian style
      ...
      // swallowtail tips (燕尾)
      parts.push(cone(0.12, 0.35, 5, 0x404030, { x: -1.3, y: 1.95, rz: 0.4 }));
      parts.push(cone(0.12, 0.35, 5, 0x404030, { x: 1.3, y: 1.95, rz: -0.4 }));
      return finish(parts);
    },
  },

  /* ---- slot 9 (CHUNK LANDMARK) ---- 宗祠牌坊 — ancestral hall pailou gateway */
  {
    id: 'ancestral_pailou',
    displayName: '宗祠牌坊',
    tier: 2,
    ...
    buildGeometry(rng) {
      const red = 0xffffff;
      const gold = 0xf0c040;
      const stone = 0xa09888;
      const parts = [];
      // stone base platform
      parts.push(box(2.2, 0.25, 1.2, stone, { y: 0.12, hex2: 0xb0a898 }));
      // two red pillars
      parts.push(cyl(0.15, 0.18, 2.2, 8, red, { x: -0.8, y: 1.3, hex2: 0xe05040 }));
      parts.push(cyl(0.15, 0.18, 2.2, 8, red, { x: 0.8, y: 1.3, hex2: 0xe05040 }));
      // pillar bases
      parts.push(box(0.35, 0.15, 0.35, stone, { x: -0.8, y: 0.32 }));
      parts.push(box(0.35, 0.15, 0.35, stone, { x: 0.8, y: 0.32 }));
      // cross beam
      parts.push(box(2.0, 0.25, 0.3, gold, { y: 2.35 }));
      // plaque board
      parts.push(box(1.2, 0.5, 0.1, 0x2a1a10, { y: 2.35 }));
      // gold trim on plaque
      parts.push(box(1.25, 0.08, 0.12, gold, { y: 2.58 }));
      parts.push(box(1.25, 0.08, 0.12, gold, { y: 2.12 }));
      // roof structure
      parts.push(box(2.4, 0.12, 0.5, 0x1f5c3a, { y: 2.6, z: 0.2, rx: -0.2 }));
      parts.push(box(2.4, 0.12, 0.5, 0x1f5c3a, { y: 2.6, z: -0.2, rx: 0.2 }));
      // ridge with swallowtail
      parts.push(box(2.4, 0.08, 0.12, gold, { y: 2.75 }));
      parts.push(cone(0.1, 0.3, 5, gold, { x: -1.2, y: 2.85, rz: 0.4 }));
      parts.push(cone(0.1, 0.3, 5, gold, { x: 1.2, y: 2.85, rz: -0.4 }));
      // center finial
      parts.push(sph(0.1, gold, { ws: 6, hs: 4, y: 2.82 }));
      return finish(parts);
    },
  },
];
