/**
 * @file packs/kinmen/archetypes/t5.js — Roll Formosa Kinmen pack, TIER 5
 ...
 * in packs/kinmen/tiers.js — spelling must NOT drift.
 *
...
export const T5_ARCHETYPES = [
  ...
  /* ---- slot 1: 坑道入口 (tunnel entrance) ------------------------------- */
  {
    ...
    collisionScale: 0.75,
    buildGeometry(rng) {
      // Tunnel entrance: concrete portal into hillside with blast doors (Kinmen style)
      const parts = [
        // hillside / rock mass
  ...
  /* ---- slot 2: 碉堡 (bunker / pillbox) ---------------------------------- */
  {
    ...
    collisionScale: 0.85,
    buildGeometry(rng) {
      // Concrete bunker/pillbox with gun slits (Kinmen coastal defense)
      const parts = [
  ...
  /* ---- slot 5: 反空降樁 (anti-landing stake / anti-airborne post) ------- */
  {
    id: 'anti_landing_stake',
    displayName: '反空降樁',
    tier: TIER,
    naturalBand: TIER,
    radiusNominal: 16,
    radiusJitter: 0.18,
    spawnWeight: 1.0,
    palette: [0x8a9098, 0x6a7078, 0x5a6068, 0xc83828, 0x4a5058],
    yOffset: -0.55,
    upright: true,
    collisionScale: 0.6,
    buildGeometry(rng) {
      // Anti-landing stakes field (反空降樁 — iconic Kinmen defense)
      const parts = [
        // ground base
        box(4.0, 0.15, 3.0, 0x5a6a5a, { y: 0.08 }),
      ];
      // simplified grid of pointed concrete stakes (5 stakes)
      const stakePos = [
        [-1.2, -0.8], [1.2, -0.8],
        [0.0, 0.0],
        [-1.2, 0.8], [1.2, 0.8],
      ];
      for (const [x, z] of stakePos) {
        // tapered concrete stake with pointed top (4 segments for low tri)
        parts.push(cyl(0.22, 0.28, 1.8, 4, 0x8a9098, { x, z, y: 1.05 }));
        parts.push(cone(0.24, 0.4, 4, 0x6a7078, { x, z, y: 2.1 })); // pointed tip
      }
      // red warning band on corner stakes
      parts.push(cyl(0.25, 0.25, 0.15, 4, 0xc83828, { x: -1.2, z: -0.8, y: 1.6 }));
      parts.push(cyl(0.25, 0.25, 0.15, 4, 0xc83828, { x: 1.2, z: 0.8, y: 1.6 }));
      // warning sign
      parts.push(cyl(0.05, 0.05, 1.4, 4, 0x6a7078, { x: 1.8, z: 0, y: 0.7 }));
      parts.push(box(0.5, 0.35, 0.05, 0xe0a030, { x: 1.8, z: 0.04, y: 1.55 }));
      return finish(parts);
    },
  },
  ...
  /* ---- slot 8: 坑道大門 (chunk landmark — large tunnel gate) ------------ */
  {
    ...
    collisionScale: 0.8,
    buildGeometry(rng) {
      // Large military tunnel entrance (like Zhaishan Tunnel entrance)
      const parts = [
        // rock / cliff mass
  ...
  /* ---- slot 9: 軍事哨站 (chunk landmark — military outpost) ------------- */
  {
    ...
    collisionScale: 0.8,
    buildGeometry(rng) {
      // Military outpost complex (Kinmen garrison style)
      const parts = [
        ...
        // sandbag barrier
        box(1.5, 0.4, 0.4, 0x8a7a5a, { x: 0, z: 2.2, y: 0.25 }),
        box(1.3, 0.3, 0.35, 0x8a7a5a, { x: 0, z: 2.2, y: 0.55 }),
      ];
      return finish(parts);
    },
  },
