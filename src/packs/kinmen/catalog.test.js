...
describe('kinmen chunk archetypes', () => {
...
describe('kinmen catalog surface', () => {
  it('merges chunk + EXTRA into a full CATALOG keyed by id (99 ids resolve)', () => {
    ...
    // Kinmen collectibles resolve (codes 70 + 94).
    expect(CATALOG['black_bear']).toBeDefined();
    expect(CATALOG['mazu']).toBeDefined();
    // Kinmen landmark resolves.
    expect(CATALOG['zhaishan_tunnel']).toBeDefined();
  });
  ...
  it('DISPLAY_NAME_BY_CODE has zh-TW names at every code (de-Tokyo: no Tokyo names left)', () => {
    ...
    // code 93 is a Kinmen extended landmark.
    expect(DISPLAY_NAME_BY_CODE[93]).toMatch(/[\u4e00-\u9fff]/); // contains Chinese characters
  });
});
