...
describe('hualien chunk archetypes', () => {
...
describe('hualien catalog surface', () => {
  it('merges chunk + Hualien EXTRA into a full CATALOG keyed by id (99 ids resolve)', () => {
    ...
    // Hualien collectibles: black_bear (code 70), mazu (code 94).
    expect(CATALOG['black_bear']).toBeDefined();
    expect(CATALOG['mazu']).toBeDefined();
    // Hualien landmark: 東大門夜市 (code 85)
    expect(CATALOG['dongdamen_arch']).toBeDefined();
  });
  ...
  it('DISPLAY_NAME_BY_CODE has zh-TW names at every code (de-Tokyo: no Tokyo names left)', () => {
    ...
    // code 0 = T0 slot 0 = qixingtan_pebble = 七星潭鵝卵石
    expect(DISPLAY_NAME_BY_CODE[0]).toBe('七星潭鵝卵石');
    ...
    // code 93 = Hualien extended landmark (東大門夜市)
    expect(DISPLAY_NAME_BY_CODE[93]).toBe('東大門夜市');
  });
});
