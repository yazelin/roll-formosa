/**
 * @file packs/hualien/archetypes/t4.js — Roll Formosa Hualien pack, TIER 4
 * content: 石藝街屋與廟 (Marble craft shops & temple).
 *
 * The 10 tier-4 ArchetypeDefs (size band 3–12 m REAL). ids are the FROZEN
 * CONTRACT from packs/hualien/tiers.js TIERS[4].archetypeIds — slots [0..7]
 ...
 *
 * Hualien T4 features marble workshops (石雕工坊), the distinctive local
 * craft industry, alongside typical Taiwan street architecture.
 */
...
export const T4_ARCHETYPES = [
  ...
  /* ---- slot 1: 石雕工坊 marble workshop (花蓮特色) ---------------------- */
  {
    id: 'marble_workshop',
    displayName: '石雕工坊',
    tier: 4,
    ...
    radiusJitter: 0.16,
    spawnWeight: 1.0,
    palette: [0xe8e0d8, 0xd0c8c0, 0x8a8a80, 0x6a6a60, 0xb8b0a8],
    yOffset: -0.38,
    upright: true,
    collisionScale: 0.85,
    buildGeometry(rng) {
      const parts = [];
      // main building - workshop structure
      parts.push(box(2.8, 1.6, 2.2, 0xe8e0d8, { y: 0.9, hex2: 0xd0c8c0 }));
      // corrugated roof
      parts.push(box(3.0, 0.12, 2.4, 0x8a8a80, { y: 1.78, rx: 0.08 }));
      // front display area
      parts.push(box(2.6, 0.8, 0.6, 0xf0f0e8, { y: 0.5, z: 1.2 }));
      // marble display pieces in front
      parts.push(sph(0.3, 0xe8e0d8, { x: -0.8, y: 0.95, z: 1.3, ws: 6, hs: 4 })); // marble ball
      parts.push(box(0.25, 0.5, 0.2, 0xd8d0c8, { x: 0, y: 0.75, z: 1.35 })); // small sculpture
      parts.push(cyl(0.15, 0.12, 0.6, 6, 0xe0d8d0, { x: 0.7, y: 0.8, z: 1.3 })); // vase
      // work area with stone blocks
      parts.push(box(0.6, 0.4, 0.5, 0xb0a898, { x: -1.0, y: 0.3, z: 0.2 })); // raw marble block
      parts.push(box(0.5, 0.35, 0.4, 0xc0b8b0, { x: 0.9, y: 0.27, z: -0.3 })); // another block
      // sign
      parts.push(box(1.5, 0.4, 0.08, 0x6a5a48, { y: 1.95, z: 1.1 }));
      parts.push(box(1.3, 0.28, 0.04, 0xf0e8d0, { y: 1.95, z: 1.14 })); // sign text area
      // tools/equipment
      parts.push(box(0.3, 0.8, 0.3, 0x5a5a58, { x: 1.2, y: 0.5, z: -0.8 })); // grinding machine
      return finish(parts);
    },
  },
  ...
  /* ---- slot 8: 石藝街屋量體 marble_shop_mass (chunk landmark) --------- */
  {
    id: 'marble_shop_mass',
    displayName: '石藝街屋量體',
    tier: 4,
    ...
    radiusJitter: 0.18,
    spawnWeight: 0.3,
    palette: [0xe8e0d8, 0xd0c8c0, 0xb8b0a8, 0x8a8a80, 0xc8bca8],
    yOffset: -0.42,
    upright: true,
    collisionScale: 0.85,
    buildGeometry(rng) {
      // a row of marble craft shops - Hualien's famous stone art street
      const parts = [];
      const heights = [2.0, 2.4, 2.2, 2.6];
      const xs = [-2.1, -0.7, 0.7, 2.1];
      const wallTints = [0xe8e0d8, 0xf0ead8, 0xe8e0d8, 0xf2e6dc];
      for (let i = 0; i < 4; i++) {
        const h = heights[i];
        // building bodies
        parts.push(towerBanded(1.3, h, 1.3, 3, wallTints[i], 0x44506a, 0xffd98a, rng, { x: xs[i], y: h / 2 }));
        parts.push(box(1.36, 0.1, 1.36, 0x8a8f9a, { x: xs[i], y: h + 0.05 })); // roof slab
        // marble display pieces in front
        if (i === 0) parts.push(sph(0.25, 0xe0d8d0, { x: xs[i] + 0.3, y: 0.35, z: 0.7, ws: 6, hs: 4 }));
        if (i === 2) parts.push(cyl(0.12, 0.1, 0.45, 6, 0xd8d0c8, { x: xs[i], y: 0.32, z: 0.7 }));
      }
      ...
      // street-front marble shop sign band
      parts.push(box(5.6, 0.2, 0.06, 0x8a7a68, { y: 0.96, z: 0.7 }));
      return finish(parts);
    },
  },
