...
describe('hsinchu chunk archetypes', () => {
...
describe('hsinchu catalog surface', () => {
  it('merges chunk + EXTRA into a full CATALOG keyed by id (99 ids resolve)', () => {
    ...
    // Hsinchu collectibles resolve (codes 78 + 94).
    expect(CATALOG['black_bear']).toBeDefined();
    expect(CATALOG['mazu']).toBeDefined();
    // Hsinchu extended landmark (code 93 = hsinchu_zoo_ext).
    expect(CATALOG['hsinchu_zoo_ext']).toBeDefined();
  });
  ...
  it('DISPLAY_NAME_BY_CODE has zh-TW names at every code (de-Tokyo: no Tokyo names left)', () => {
    ...
    // code 0 = T0 slot 0 = hsinchu_glass_marble = 新竹玻璃珠
    expect(DISPLAY_NAME_BY_CODE[0]).toBe('新竹玻璃珠');
    ...
    // code 93 is now a Hsinchu extended landmark (hsinchu_zoo_ext).
    expect(DISPLAY_NAME_BY_CODE[93]).toBe('新竹動物園');
  });
});
