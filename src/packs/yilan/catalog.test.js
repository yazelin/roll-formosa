...
describe('yilan chunk archetypes', () => {
...
describe('yilan catalog surface', () => {
  it('merges chunk + Yilan EXTRA into a full CATALOG keyed by id (99 ids resolve)', () => {
    ...
    // Yilan collectibles (codes 70..81+94) and landmarks (codes 82..88) resolve.
    expect(CATALOG['col_sanxing_scallion']).toBeDefined();
    expect(CATALOG['col_guishan_turtle']).toBeDefined();
    expect(CATALOG['jimmy_plaza']).toBeDefined();
  });
  ...
  it('DISPLAY_NAME_BY_CODE has zh-TW names at every code (no Japanese kana)', () => {
    ...
    // Yilan landmark name at code 82.
    expect(DISPLAY_NAME_BY_CODE[82]).toBe('幾米廣場');
  });
});
