...
describe('penghu chunk archetypes', () => {
...
describe('penghu catalog surface', () => {
  it('merges chunk + EXTRA into a full CATALOG keyed by id (unique ids resolve)', () => {
    // Penghu: 70 chunk + 8 landmarks + 13 collectibles + 9 extended = 100 but some reuse
    // ids so we expect at least 70+8+13+1(goal) = 92+ unique ids.
    // The exact count depends on how many extended slots reuse primary landmark ids.
    ...
    // P7: Penghu collectibles resolve (codes 70 + 94).
    expect(CATALOG['black_bear']).toBeDefined();
    expect(CATALOG['mazu']).toBeDefined();
    // Penghu landmarks resolve.
    expect(CATALOG['double_heart_weir']).toBeDefined();
    expect(CATALOG['cross_sea_bridge']).toBeDefined();
  });
  ...
  it('DISPLAY_NAME_BY_CODE has zh-TW names at every code (de-Tokyo: no Tokyo names left)', () => {
    ...
    // code 0 = T0 slot 0 = coral_bead = 珊瑚珠
    expect(DISPLAY_NAME_BY_CODE[0]).toBe('珊瑚珠');
    ...
    // code 93 is the Penghu goal slot (跨海大橋).
    expect(DISPLAY_NAME_BY_CODE[93]).toBe('澎湖跨海大橋');
  });
});
