...
import { CHUNK_ARCHETYPES, CATALOG, DISPLAY_NAME_BY_CODE, EXTRA_CATALOG } from './catalog.js';
...
describe('taitung chunk archetypes', () => {
...
describe('taitung catalog surface', () => {
  it('has all 70 chunk archetypes in CATALOG', () => {
    for (const id of TIERS.flatMap((t) => t.archetypeIds)) {
      expect(CATALOG[id], `chunk id '${id}' missing from CATALOG`).toBeDefined();
    }
  });

  it('EXTRA_CATALOG covers all 29 EXTRA codes (70..98)', () => {
    // 70..81 collectibles, 82..89 landmarks, 90..93 extended, 94 mazu, 95..98 extended
    for (let code = 70; code <= 98; code++) {
      expect(EXTRA_CATALOG[code], `EXTRA code ${code} missing`).toBeDefined();
    }
    expect(Object.keys(EXTRA_CATALOG).length).toBe(29);
  });

  it('Taitung collectibles and landmarks resolve in CATALOG', () => {
    // P7: Taitung collectibles (codes 70..81 + 94)
    expect(CATALOG['col_sugar_apple']).toBeDefined();
    expect(CATALOG['flying_fish']).toBeDefined();
    expect(CATALOG['mazu']).toBeDefined();
    // Taitung landmarks (codes 82..89)
    expect(CATALOG['tiehua_village']).toBeDefined();
    expect(CATALOG['sanxiantai_bridge_ext93']).toBeDefined();
  });
  ...
  it('DISPLAY_NAME_BY_CODE has zh-TW names at every code (no Japanese kana)', () => {
    ...
    // code 93 is Taitung extended landmark (三仙台八拱橋).
    expect(DISPLAY_NAME_BY_CODE[93]).toBe('三仙台八拱橋');
  });
});
