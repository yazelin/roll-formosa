/**
 * @file packs/taitung/archetypes/t4.js — Roll Formosa Taitung pack, TIER 4
 * content: 部落街屋與廟 (tribal shophouses & temples).
 *
 * The 10 tier-4 ArchetypeDefs (size band 3–12 m REAL). ids are the FROZEN
 * CONTRACT from packs/taitung/tiers.js TIERS[4].archetypeIds — slots [0..7]
...
export const T4_ARCHETYPES = [
  /* ---- slot 0: 透天厝 (narrow 3–4 storey townhouse) -------------------- */
  {
    ...
    buildGeometry(rng) {
      const parts = [
        // tall narrow banded body, tinted plaster + warm windows
  ...
  /* ---- slot 1: 鐵皮屋 (low corrugated-tin shed) ------------------------ */
  {
    ...
    buildGeometry(rng) {
      const parts = [
        box(2.4, 1.0, 1.7, 0xd8d4cc, { y: 0.5 }), // plaster lower walls
        box(2.42, 0.5, 1.72, 0xffffff, { y: 1.2, hex2: 0xe0d8d0 }), // upper tin walls
        // single-slope corrugated tin roof
  ...
  /* ---- slot 2: 部落公寓 (tribal-style apartment building) -------------- */
  {
    id: 'tribal_apartment',
    displayName: '部落公寓',
    tier: 4,
    ...
    radiusJitter: 0.15,
    spawnWeight: 1.0,
    palette: [0xe2d8c8, 0xd0d8e0, 0xc04030, 0xf0c030, 0xc8bca8],
    yOffset: -0.28,
    upright: true,
    collisionScale: 0.85,
    buildGeometry(rng) {
      const parts = [
        ...
        // tribal pattern band along facade
        box(2.1, 0.2, 0.06, 0xc04030, { y: 1.8, z: 0.68 }),
        // rooftop clutter
      ...
      // triangular tribal motifs
      for (let i = 0; i < 5; i++) {
        parts.push(box(0.2, 0.15, 0.04, i % 2 ? 0xf0c030 : 0xc04030, { x: -0.8 + i * 0.4, y: 1.8, z: 0.72 }));
      }
      // 鐵窗 cages on each floor
      for (let f = 0; f < 4; f++) {
        const y = 0.9 + f * 0.5;
  ...
  /* ---- slot 8: 部落街屋 (chunk landmark — tribal streethouse mass) --- */
  {
    ...
    radiusJitter: 0.18,
    spawnWeight: 0.3,
    palette: [0xc8b89a, 0xb8a888, 0xa89878, 0xc04030, 0xf0c030],
    yOffset: -0.5417,
    upright: true,
    collisionScale: 0.85,
    buildGeometry(rng) {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        parts.push(box(1.42, 0.08, 1.46, 0x6a5a48, { x: xs[i], y: h + 0.02 }));
        // tribal decorative band
        parts.push(box(1.35, 0.15, 0.04, 0xc04030, { x: xs[i], y: h * 0.75, z: 0.67 }));
        parts.push(box(0.34, 0.3, 0.05, 0x4a463c, { x: xs[i] - 0.28, y: h * 0.5, z: 0.67 }));
        parts.push(box(0.34, 0.3, 0.05, 0x4a463c, { x: xs[i] + 0.28, y: h * 0.5, z: 0.67 }));
      }
  ...
  /* ---- slot 9: 天后宮 (chunk landmark — Mazu temple complex) -------- */
  {
    ...
    radiusJitter: 0.15,
    spawnWeight: 0.3,
    palette: [0xc04030, 0xb83828, 0xf0c030, 0xa88a5a, 0x8a7a5a],
    yOffset: -0.4781,
    upright: true,
    collisionScale: 0.85,
    buildGeometry(rng) {
      const parts = [
        ...
        // red-walled main hall
        box(3.0, 1.4, 1.8, 0xffffff, { y: 1.1, hex2: 0xc04030 }),
      ];
      // big dark tiled hip-and-gable roof
      parts.push(cyl(1.5, 1.5, 3.6, 4, 0x3a4048, { theta0: PI, rx: HALF_PI, sy: 0.55, y: 2.0 }));
      parts.push(box(3.6, 0.12, 2.2, 0x4a505a, { y: 1.82 }));
      parts.push(box(3.2, 0.16, 0.22, 0x2e343c, { y: 2.55 }));
      // upturned ridge ends (swallowtail style)
      parts.push(cone(0.2, 0.5, 5, 0xf0c030, { x: -1.7, y: 2.65, rz: 0.5 }));
      parts.push(cone(0.2, 0.5, 5, 0xf0c030, { x: 1.7, y: 2.65, rz: -0.5 }));
      // entrance doors
      parts.push(box(0.9, 0.9, 0.08, 0x6a2a1a, { y: 0.85, z: 0.92 }));
      // red boundary wall around the complex
      parts.push(box(4.4, 0.5, 0.12, 0xffffff, { y: 0.45, z: 1.6, hex2: 0xb83828 }));
      parts.push(box(4.4, 0.5, 0.12, 0xffffff, { y: 0.45, z: -1.6, hex2: 0xb83828 }));
      parts.push(box(0.12, 0.5, 3.3, 0xffffff, { x: -2.2, y: 0.45, hex2: 0xb83828 }));
      parts.push(box(0.12, 0.5, 3.3, 0xffffff, { x: 2.2, y: 0.45, hex2: 0xb83828 }));
      // wall coping caps
      parts.push(box(4.44, 0.08, 0.18, 0x3a4048, { y: 0.72, z: 1.6 }));
      parts.push(box(4.44, 0.08, 0.18, 0x3a4048, { y: 0.72, z: -1.6 }));
      // dragon pillar at entrance
      parts.push(cyl(0.15, 0.18, 1.2, 6, 0xc04030, { x: -0.7, y: 0.8, z: 1.0 }));
      parts.push(cyl(0.15, 0.18, 1.2, 6, 0xc04030, { x: 0.7, y: 0.8, z: 1.0 }));
      return finish(parts);
    },
  },
