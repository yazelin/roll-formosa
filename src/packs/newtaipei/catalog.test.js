...
describe('newtaipei chunk archetypes', () => {
...
describe('newtaipei catalog surface', () => {
  it('merges chunk + EXTRA into a full CATALOG keyed by id (70 chunk + collectibles + landmarks)', () => {
    // 70 chunk + 13 collectibles + 8 landmarks + 1 goal + 7 placeholders = 99 archetype ids.
    // But catalog may have fewer unique entries if some codes share ids - check structure only.
    for (const id of TIERS.flatMap((t) => t.archetypeIds)) {
      expect(CATALOG[id], `chunk id '${id}' missing from CATALOG`).toBeDefined();
    }
    // New Taipei collectibles resolve (codes 70..81 + 94).
    expect(CATALOG['agei']).toBeDefined();
    expect(CATALOG['mazu']).toBeDefined();
    // New Taipei landmarks resolve (codes 82..89).
    expect(CATALOG['jiufen_teahouse']).toBeDefined();
  });
  ...
  it('DISPLAY_NAME_BY_CODE has zh-TW names at every code (de-Tokyo: no Tokyo names left)', () => {
    ...
    // code 0 = T0 slot 0 = yingge_pottery_shard = 鶯歌陶片
    expect(DISPLAY_NAME_BY_CODE[0]).toBe('鶯歌陶片');
    ...
    // code 93 is the goal display slot (淡水漁人碼頭情人橋).
    expect(DISPLAY_NAME_BY_CODE[93]).toBe('淡水漁人碼頭情人橋');
  });
});
