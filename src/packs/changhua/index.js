/**
 * @file changhua/index.js — Changhua StagePack.
 *
 * Full content surface required by R16. Wires 8 curated Changhua landmarks
 * (鹿港龍山寺/鹿港天后宮/扇形車庫/彰化孔廟/摸乳巷/九曲巷/玻璃廟/王功燈塔)
 * into the cityMap and catalog so they spawn in the world and can be absorbed.
 * Goal monument is 八卦山大佛.
 *
 * Pack identity overrides: id='changhua', displayName='彰化', region='TW',
 * locale=zh-TW. Seeds are Changhua-specific (CHAN = 0x4348414E,
 * v5 extension = 0x6368616E).
 *
 * Validation: routes through validatePack(pack) (R6). Changhua LANDMARKS carry
 * native `name` and `isGoal` fields — the legacy shim is removed.
 *
 * Code-map methods (R5): attached on the pack object via buildCodeMap.
 */

import { TIERS, RESCALE_S, ARCH_PER_TIER, validateTiersStructure } from './tiers.js';
import {
  CATALOG,
  EXTRA_CATALOG,
  DISPLAY_NAME_BY_CODE,
  EXTRA_SIZE_CLASS_BY_CODE,
  EXTRA_POOL_CAPS,
} from './catalog.js';
import { EXTRA_ARCHETYPE_IDS, V5_ARCHETYPE_IDS } from '../../world/objects.js';
import * as cityMap from './cityMap.js';
import { ABSORB_RATIO, MAP_BOUNDS } from '../../config/tuning.js';
import { buildCodeMap, validatePack } from '../_engine/codeMap.js';
import { locale } from './locale.js';
import { goalMonument } from './monument.js';
import * as narration from './narration.js';
import { ending } from './ending.js';

// Changhua replaces EXTRA codes 82..89 with the 8 core Changhua landmark ids.
// Collectible codes 70..81 (indices 0..11) carry Changhua collectibles.
// Codes 90..93/94..98 carry extended Changhua landmarks.
import { NM_LUKANG_LONGSHAN } from './landmarks/lukang_longshan.js';
import { NM_LUKANG_TIANHOU } from './landmarks/lukang_tianhou.js';
import { NM_ROUNDHOUSE } from './landmarks/roundhouse.js';
import { NM_CHANGHUA_CONFUCIUS } from './landmarks/changhua_confucius.js';
import { NM_MORUXIANG } from './landmarks/moruxiang.js';
import { NM_JIUQUXIANG } from './landmarks/jiuquxiang.js';
import { NM_GLASS_TEMPLE } from './landmarks/glass_temple.js';
import { NM_WANGGONG_LIGHTHOUSE } from './landmarks/wanggong_lighthouse.js';

/**
 * EXTRA id order for the Changhua pack.
 * Codes 70..81 (indices 0..11): Changhua collectible ids.
 * Codes 82..89 (indices 12..19): Changhua core landmark ids.
 * Codes 90..93 (indices 20..23): Changhua extended landmark ids.
 * Codes 94..98 (v5 indices 0..4): 媽祖 + 4 extended landmark ids.
 */
const extraIds = [
  // indices 0..11 — codes 70..81: Changhua collectibles
  'black_bear', 'meatball', 'kongrou_rice', 'oyster_omelet', 'ox_tongue_cake', 'phoenix_eye_cake',
  'misua', 'face_tea', 'old_locomotive', 'paper_fan_collectible', 'lantern', 'lugang_incense',
  // indices 12..19 — codes 82..89: Changhua core landmark ids
  NM_LUKANG_LONGSHAN.id,     // 82 鹿港龍山寺
  NM_LUKANG_TIANHOU.id,      // 83 鹿港天后宮
  NM_ROUNDHOUSE.id,          // 84 扇形車庫
  NM_CHANGHUA_CONFUCIUS.id,  // 85 彰化孔廟
  NM_MORUXIANG.id,           // 86 摸乳巷
  NM_JIUQUXIANG.id,          // 87 九曲巷
  NM_GLASS_TEMPLE.id,        // 88 鹿港玻璃廟
  NM_WANGGONG_LIGHTHOUSE.id, // 89 王功燈塔
  // indices 20..23 — codes 90..93: Changhua extended landmarks
  'yuanlin_station', 'lukang_old_street', 'changhua_station', 'baguashan_skyway',
  // v5 codes 94..98 — 94 媽祖; 95..98 Changhua extended landmarks
  'mazu', 'tianwei_highway_garden', 'lukang_wenkai_academy', 'lukang_first_street', 'xiluo_bridge',
];

// Collectible album id -> index in extraIds (legacy rule preserved).
const collectibleExtraIndex = {};
for (let id = 0; id <= 11; id++) collectibleExtraIndex[id] = id;
collectibleExtraIndex[12] = EXTRA_ARCHETYPE_IDS.length + 0; // mazu (first v5)

/** @type {import('../_engine/codeMap.js').StagePack} */
export const activePack = {
  id: 'changhua',
  displayName: '彰化',
  region: 'TW',
  locale, // zh-TW t()/fmt() (R10: routes HUD/screens strings)
  tiers: TIERS, // Changhua tier ladder
  rescaleS: RESCALE_S,
  archPerTier: ARCH_PER_TIER,
  archetypes: CATALOG, // Changhua archetype recipes
  extraCatalog: EXTRA_CATALOG,
  extraSizeClassByCode: EXTRA_SIZE_CLASS_BY_CODE,
  extraPoolCaps: EXTRA_POOL_CAPS,
  displayNameByCode: DISPLAY_NAME_BY_CODE, // P5 replaces with zh-TW names
  extraIds,
  collectibleExtraIndex,
  cityMap, // Changhua city map namespace
  map: { bounds: { x: MAP_BOUNDS.x, z: MAP_BOUNDS.z } },
  landmarks: cityMap.LANDMARKS, // Changhua landmarks
  absorbRatio: ABSORB_RATIO,
  seeds: { primary: 0x4348414E, v5: 0x6368616E }, // CHAN / chan
  goalMonument, // 八卦山大佛 goal monument (buildGeometry/pos/winToast)
  narration,    // 月牙 zh-TW narration tables
  ending,       // v6: Formosa-island reveal definition
  validate() {
    // P4: structural ladder invariants (no catalog dependency).
    validateTiersStructure();

    // Changhua LANDMARKS carry native name/isGoal — run the real
    // validatePack directly with no shim. The 9-entry ladder
    // (8 curated + 八卦山大佛 goal) must pass the strictly-increasing threshold
    // check and the goal-is-largest invariant.
    validatePack(this);
    return true;
  },
};

/**
 * Pack-scoped code-map methods (R5), built once by buildCodeMap and attached
 * to the pack object so consumers read them off `activePack`.
 */
const _codeMap = buildCodeMap(activePack);
/** @type {string[]} code -> archetype id. */
activePack.archetypeIdByCode = _codeMap.idByCode;
/** @type {Record<string, number>} archetype id -> code. */
activePack.codeByArchetypeId = _codeMap.codeById;
/** @type {string[]} code -> display name (Changhua zh-TW names). */
activePack.displayNameByCodeArr = DISPLAY_NAME_BY_CODE;
/** @param {number} id collectible album id @returns {number} archetype code. */
activePack.codeForCollectibleId = (id) => _codeMap.collectibleCodeForId(id);
/** @param {number} code archetype code @returns {string} archetype id. */
activePack.codeToArchetypeId = (code) => _codeMap.idByCode[code];

export default activePack;
