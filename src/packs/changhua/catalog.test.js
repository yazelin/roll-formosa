...
describe('changhua chunk archetypes', () => {
...
describe('changhua catalog surface', () => {
  it('merges chunk + Changhua EXTRA into a full CATALOG keyed by id (99 ids resolve)', () => {
    ...
    // P7: Changhua collectibles resolve (codes 70 + 94).
    expect(CATALOG['black_bear']).toBeDefined();
    expect(CATALOG['mazu']).toBeDefined();
    // Changhua extended landmark at code 93.
    expect(CATALOG['baguashan_skyway']).toBeDefined();
  });
  ...
  it('DISPLAY_NAME_BY_CODE has zh-TW names at every code (de-Tokyo: no Tokyo names left)', () => {
    ...
    // code 0 = T0 slot 0 = flour_tea_packet = 麵茶包
    expect(DISPLAY_NAME_BY_CODE[0]).toBe('麵茶包');
    ...
    // code 93 is a Changhua extended landmark.
    expect(DISPLAY_NAME_BY_CODE[93]).toBe('八卦山天空步道');
  });
});
